 ...
 *
 * A CALIBRER avant la demo : la lecture du capteur de courant (voir readWatts()).
 *
 * --- ARDUINO IDE ---
 * Board : "ESP32 Dev Module"
 * Librairies a installer via le Library Manager :
 *   1) WebSockets  (par Markus Sattler)
...
const char* WIFI_SSID     = "Fiberbox-5G-46EC";   // nom du WiFi (le MEME que le PC) - ESP32 = 2.4 GHz uniquement
const char* WIFI_PASS     = "2e6446ec";          // mot de passe du WiFi
const char* BACKEND_HOST  = "192.168.1.12";        // IP locale du PC qui lance le backend
const uint16_t BACKEND_PORT = 8000;                // port du backend

const char* PRISE_ID       = "p2";                 // identifiant physique de la prise
const char* DEVICE_ID      = "kettle-1";           // appareil branche dessus
const char* DEVICE_NAME    = "Bouilloire";         // nom affiche (auto-decouverte)
const char* DEVICE_PRIORITE = "reportable";        // essentiel | reportable | confort

const int   PIN_RELAY     = 26;                    // sortie vers le module relais
const int   PIN_SENSOR    = 34;                    // entree analogique du capteur de courant
const unsigned long MEASURE_INTERVAL_MS = 1000;

// Calibration capteur (SCT-013 + circuit de mesure). A ajuster avec une charge connue.
const float MAINS_VOLTAGE = 230.0;                 // tension secteur nominale
const float AMPS_PER_ADC  = 0.0;                   // TODO: calibrer (A par unite ADC)
...
void setRelay(bool on) {
  relayOn = on;
  digitalWrite(PIN_RELAY, on ? HIGH : LOW);        // adapter selon module (actif haut/bas)
  Serial.printf("[relay] %s\n", on ? "ON" : "OFF");
}

// Lit la puissance instantanee (W). Placeholder a calibrer.
float readWatts() {
  if (!relayOn) return 0.0;                        // coupe = pas de conso
...
void onWsEvent(WStype_t type, uint8_t* payload, size_t length) {
  switch (type) {
    ...
    case WStype_TEXT: {
      ...
      if (err) { Serial.println("[ws] JSON invalide"); return; }
